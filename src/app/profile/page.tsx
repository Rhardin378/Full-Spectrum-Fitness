...
export default async function ProfilePage() {
  ...
  if (!profileResult.success) {
    ...
    return (
      <main className="min-h-screen bg-slate-950 p-6 text-slate-100 sm:p-10">
        <section className="mx-auto max-w-3xl rounded-2xl border border-rose-500/30 bg-rose-500/10 p-8">
  ...
  return (
    <main className="min-h-screen bg-slate-950 p-6 text-slate-100 sm:p-10">
      <section className="mx-auto max-w-3xl rounded-2xl border border-white/10 bg-white/5 p-8 shadow-xl">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm text-sky-300">Profile setup</p>
            <h1 className="text-3xl font-semibold">Tell us about your journey</h1>
          ...
          <Link
            href="/dashboard"
            className="inline-flex items-center justify-center rounded-xl border border-white/20 px-4 py-2 text-sm font-medium text-slate-200 transition hover:bg-white/10"
          >
            Back to dashboard
          </Link>
        </div>
